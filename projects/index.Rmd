...
Some relevant things about these metrics:
...
* We don't make videos/stories/reels which simplifies things. The data from Meta only represents Facebook, because that is where we're active. There is no way to track memes being stolen or copy/pasted to other social media sites. I'm going to completely ignore this very common practice, despite its obvious relevance to analyzing memetic virality.

* Some have the same names as similar metrics addressing performance of pages as a whole. I'm interested in individual posts, so these are metrics are on the post level.

* I will be using impressions instead of reach. In measuring the "*virality*" of posts, impressions are more useful because, as a measure of unique users, Reach is not accounting for memes being seen multiple times by the same user. Seeing something time and time again means users are more likely to engage with the meme, or some mutation of it they particularly like. Users are also more likely to reference the meme in conversation or their own posts. Although less interesting from a page admin standpoint, Impressions is better at showing social media *zeitgeist* relevance. 

* Engagement is also important. It is a combination of all direct interaction with a post (clicks, likes/reactions, comments, shares, etc.) Shares is the most interesting because it shows that a user liked the post enough to associate it with their account. Each share represents a new set of users exposed to the post.

Edit: Meta's new update lets you download data relative to the day posts were made. That's interesting, but I will continue to use what I have. Many posts are not instantly successful (Ex. they might take off only once shared to a certain group). Total data is better for the things relevant to this project.
...
My Facebook is in Spanish and so is the data. This is my own fault because I set a lot of my UIs to Spanish so I can continually practice. I am fluent, but I don't necessarily know what words/phrases correspond to what in Anglo-ish. Tech vocab is often not a direct translation. There are also occasionally words from Latin American Spanish which I don't recognize, or which may be used differently in those dialects. An even larger problem is vowels with tildes "`". I typically ignore these letters when typing in Spanish (a terrible, yet common practice). Five extra letters in the alphabet means that column names can't be referred to accurately in code.

Business Suite's Insights tab gives data in .csv format. That's good, but it only lets you download three months at a time. So we have four raw uncleaned .csvs representing all the page's posts over the course of 2022. A single dataframe will be much easier to work with.
...
Edit: The 2021 and 2023 .csvs did not have 34 and had to go through select() before rbind().
...
We need a single df, so use rbind() to append all the observations together.

4. Select()

Most of those 34 variables Meta gave are irrelevant or empty. We'll keep what has data, but what we really want is impressions, engagement, and shares. Changed the order of columns to be a little less annoying too.

I also translated the column names into English. Some descriptions are still in Spanish, but we don't care about those.

5. Separate date_hour variable

The date and time of of each post are different variables and need to be separated.

They also had to be changed to POSIX format using as.POSIXct()

The class of time is still stuck on character. Trying with as.POSIX() forces it to add on a date (today's by default). May not be a problem. Will come back if it is.

6. New Variable: OC

We mark our original posts with "OC" (or some variation) in their descriptions, so we are going to need to make a new column based on the post descriptions. We can get a variable for what is or isn't OC using unique() and grep().

7. New Variable: weekday

The weekly distribution of posts may have some impact. Used the weekdays() function to add a a new variable. as.factor and set levels to put the days in order.
...
(There are likely some false negatives. I have only had time to manually fix a few important outliers. This should be a diminishingly small number including only those posts which we forgot to mark when they were originally made.)
...
It looks like there are 3 outliers consistently messing with things. Those must represent posts that went really viral. It looks like there is also a cluster of 5 or so posts that were also quite successful sitting behind them, but ahead of the pack.


### Numbers:

Lets get a sense of the overall scale of the page.
...
We'll get a better look at those top viral posts. I want to see what's doing best at doing best.
I'm arbitrarily setting the *viral* cutoff as multiples of 10 which leave ~1% of the observations for that variable.
...
We can see here that the top posts are disproportionately likely to be OC compared to the rest of the data.
Of the posts with impressions > 10,000, 15/26 are OC. Of those with engagements > 1000, 5/7 are OC.

This supports the idea that OC particularly appeals to the page's more active audience members who are more likely to interact with content.

With a full tidy data set we can already get some insight.
...
This tells us that OC is indeed performing better than shared content across a variety of metrics. 
...
I'm going to be using log10() for most things because that is how virality works. To help get a sense of the scale we're working with and why I'm doing it like this, here is a terrible figure showing the distribution of impressions without a log:
...
Ie. the figure from before, but not terrible.
...
Very cool. We've got a bit of a bimodal distribution here. Probably from the explosive growth the page had in 2021. Seems like the algorithmic reach of the page skipped right over the 10^2 range.


### Weekday Distribution:

Now I am also curious about the distribution of posts across the days of the week.
...
This weekday distribution is interesting. I left out 2021 and 2022 because they're not that interesting, but 2023 compared to the overall data is neat. It tracks my own Facebook usage because all the admins had very different schedules in spring, and I'm the one who shares the most. Wednesday and Sunday were the days I had the most time to study this semester followed by Tuesday and Thursday. I often take study breaks where I browse Facebook. Not enough time to make memes (hence less OC those days), but enough to share. 


### Prophet:

I'd like to see how much seasonality affects post performance. Facebook created the Prophet package for just this sort of thing. It makes seasonal forecast models for data sets just like this one. This is interesting because I could see it going either way. Maybe winter doesn't matter to how our performance, or maybe it matters a lot. I would have liked to test particular holidays (Prophet has a very cool data set with worldwide holidays until 2044), but our audience demographics are too globalized.
...
I really wish I had the next month's worth of data to see if the May spike is real, or if Prophet is getting veered off course by the 2021 growth spurt.
...
Now the thing is that you can turn Prophet's seasonality on and off. It doesn't seem like engagement is particularly affected by the season. Here is the non seasonal version to compare.
...
I'm holding off judgement on seasonality for impressions. The graphs are more different than engagements was. Further, engagements may reflect the activity of a more dedicated kind of social medial user who is less affected by seasonality. Most users are instead "lurkers" who do not actively interact with content. Impressions being non-unique views, they may be more affected by the impact of seasonality on lurkers.
...
* The page's audience cares about what the page produces.
  + Engagement is higher on original content, which means the page's audience is interested enough in what the page is creating to interact with it.


* The trend of the page's success is very related to the admins' level of engagement with the page.
...
And a sneak peak at our next post:
```{r}
