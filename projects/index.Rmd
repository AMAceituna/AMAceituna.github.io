...
They had to be changed to the POSIX format using as.POSIXct()

The class of time is still stuck on character. POSIX forces it to take on a date (today's by default). Might not be a problem. Will come back if it is.
...
That means a total of 693 posts. Even between three admins an original meme every day is a lot, so right off the bat you know that OC:shares is certainly not 1:1. My guess would be around 3:1/4:1.

Edit: Managed to get an OC variable. 101/693 posts are oc based on the tidying. Will make more figures exploring this later. There is at least one incorrectly labeled OC.
...
Here's a quick histogram of the data so far. That top observation is actually an unmarked OC David made. Will need to go back through and check what is not marked correctly. Manual editing, but it's still technically part of data collection, I think.
